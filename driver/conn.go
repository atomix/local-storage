...
func newConn(network network.Network) *memoryConn {
	return &memoryConn{
...
func (c *memoryConn) Connect(ctx context.Context, spec runtime.ConnSpec) error {
...
func (c *memoryConn) Configure(ctx context.Context, spec runtime.ConnSpec) error {
	var config protocol.ProtocolConfig
	if err := spec.UnmarshalConfig(&config); err != nil {
		return err
	}
...
func (c *memoryConn) NewCounter(spec runtime.PrimitiveSpec) (counterv1api.CounterServer, error) {
	return counterv1.NewCounterProxy(c.Protocol, spec)
}

func (c *memoryConn) NewCounterMap(spec runtime.PrimitiveSpec) (countermapv1api.CounterMapServer, error) {
	return countermapv1.NewCounterMapProxy(c.Protocol, spec)
}

func (c *memoryConn) NewLeaderElection(spec runtime.PrimitiveSpec) (electionv1api.LeaderElectionServer, error) {
	return electionv1.NewLeaderElectionProxy(c.Protocol, spec)
}

func (c *memoryConn) NewIndexedMap(spec runtime.PrimitiveSpec) (indexedmapv1api.IndexedMapServer, error) {
	return indexedmapv1.NewIndexedMapProxy(c.Protocol, spec)
}

func (c *memoryConn) NewLock(spec runtime.PrimitiveSpec) (lockv1api.LockServer, error) {
	return lockv1.NewLockProxy(c.Protocol, spec)
}

func (c *memoryConn) NewMap(spec runtime.PrimitiveSpec) (mapv1api.MapServer, error) {
	return mapv1.NewMapProxy(c.Protocol, spec)
}

func (c *memoryConn) NewMultiMap(spec runtime.PrimitiveSpec) (multimapv1api.MultiMapServer, error) {
	return multimapv1.NewMultiMapProxy(c.Protocol, spec)
}

func (c *memoryConn) NewSet(spec runtime.PrimitiveSpec) (setv1api.SetServer, error) {
	return setv1.NewSetProxy(c.Protocol, spec)
}

func (c *memoryConn) NewValue(spec runtime.PrimitiveSpec) (valuev1api.ValueServer, error) {
